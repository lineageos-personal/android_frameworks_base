...
class NetworkSpeedView @JvmOverloads constructor(
    ...
    companion object {
        ...
        fun fromContext(context: Context, slot: String, blocked: Boolean, controller: NetworkSpeedController): NetworkSpeedView {
            val view = LayoutInflater.from(context)
                .inflate(R.layout.network_speed_view, null) as NetworkSpeedView
            view.setSlot(slot)
            view.initDotView()
            view.isBlocked = blocked
            view.networkSpeedController = controller
            return view
        }
    }

    private var networkSpeedController: NetworkSpeedController? = null
    ...
    override fun isIconBlocked(): Boolean {
        return isBlocked && !(networkSpeedController?.isSwitchOn() ?: false)
    }
}
